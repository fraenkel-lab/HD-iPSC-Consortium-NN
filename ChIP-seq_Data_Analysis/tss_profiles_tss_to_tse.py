...
def TSS_dist(sam_files,TSSfile,genome,outdir,pos=False,genes=False,shift=90,bin=20,min_tags=40,mv_avg=False,norm=False,format='SAM'):
    ...
    get_dist(SAM,TSS,outdir,n_reads,genes=genes,bins=bin,min_tags=min_tags,mv_avg=mv_avg,norm=norm)
        
def get_dist(SAM,TSS,outdir,n_reads,genes=False,bins=None,min_tags=40,mv_avg=False,norm='density'):
...
def BIN(X,bins,mv_avg=False):
    k=len(X)//bins
    #Switched k for bins so k is the amt of bp/bin and bins is the amt of bins 
    binned=[np.sum(X[(i*k):((i+1)*k)]) for i in xrange(bins)]
    if mv_avg:
...
def main():
    ...
    TSS_dist(sam_file,options.TSSfile,genome,outdir,pos=options.pos,genes=options.genes,shift=options.shift,bin=options.bin,min_tags=options.min_tags,mv_avg=options.mv_avg,norm=options.norm,format=options.format)
